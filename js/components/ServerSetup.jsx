...
var ServerSetup = React.createClass({
    ...
    _assignIdsAndParents() {
        /*
        Need to have unique ids set on services, characteristics and descriptors.
        Also need to set the parent property. We have to redo this whenever the tree changes.
        */
        var id = 0
        Array.unobserve(this.services, this._assignIdsAndParents);
        Array.observe(this.services, this._assignIdsAndParents);
        this.services.forEach(service => {
            service.id = id++;
            Array.unobserve(service.characteristics, this._assignIdsAndParents);
            Array.observe(service.characteristics, this._assignIdsAndParents);
            service.characteristics.forEach(characteristic => {
                characteristic.id = id++;
                characteristic.parent = service;
                Array.unobserve(characteristic.descriptors, this._assignIdsAndParents);
                Array.observe(characteristic.descriptors, this._assignIdsAndParents);
    ...
    _addService() {
        var service = { "handle": 0, "uuid": "", "name": "New service", "characteristics": [], "serviceUuid": "" };
