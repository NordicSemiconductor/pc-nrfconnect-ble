...
var connectionStore = reflux.createStore({
    ...
    onConnectToDevice: function(device) {
        ...
        logger.info(`Initiated connection to peripheral ${device.peer_addr.address}.`);
        var scanParameters = {
        ...
        bleDriver.gap_connect(device.peer_addr, scanParameters, connectionParameters, function(err) {
            if(err) {
                logger.error(`Could not connect to peripheral ${device.peer_addr.address} due to error. ${err}`);
                bleDriver.gap_cancel_connect(function(err) {
                    if (err) {
                        logger.error(`Could not cancel connection to peripheral ${device.peer_addr.address}`);
                        return;
                    }
                });
            } else {
                logger.debug(`Successfully sent connection request to driver (peripheral ${device.peer_addr.address}).`);
    ...
    onDeviceConnected: function(eventPayload){
        logger.info(`Peripheral ${eventPayload.peer_addr.address} connected.`);
    ...
    onDeviceDisconnected: function(eventPayload){
        ...
        logger.info(`Peripheral ${connectionThatWasDisconnected.peer_addr.address} disconnected. Disconnect reason is ${eventPayload.reason_name}.`);
    ...
    onDisconnectFromDevice: function(deviceAddress) {
        ...
        bleDriver.gap_disconnect(connectionHandle, bleDriver.BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION, function(err){
            if(err) {
                logger.error(`Error disconnecting from peripheral ${deviceAddress}. Error is ${err}`);
                return;
            }
