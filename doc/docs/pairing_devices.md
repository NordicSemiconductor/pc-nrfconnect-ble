...
The Bluetooth Low Energy app can reply automatically to an incoming pairing request by default, or it can be handled by the user.
...
2. Select or deselect **Auto reply security requests**.<br/>
   When **Auto reply security requests** is selected, the Bluetooth Low Energy app uses the settings specified in **Security parameters** when automatically replying to the request.
